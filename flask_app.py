...
# ==============================================================================
# CONFIGURAÇÃO PRINCIPAL DO APP (FEITA APENAS UMA VEZ)
# ==============================================================================
app = Flask(__name__)
...
# CAMINHOS E ARQUIVOS
# ==============================================================================
# Caminho absoluto e seguro para o arquivo JSON de ocorrências
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
OCCURRENCES_FILE = os.path.join(BASE_DIR, 'occurrences.json')

# ==============================================================================
# DADOS ESTÁTICOS DE LOCALIZAÇÕES E RISCO
# ==============================================================================
LOCATIONS_DATA = {
    "Norte": {
        "AM": [
    ...
    "Sudeste": {
        "SP": [
            ...
            },
            {
                ...
                "schedule": {
                    "even": {"day": "Ulisses (07h às 19h)", "night": "Gabriel (19h às 07h)"},
                    "odd": {"day": "Douglas (07h às 19h)", "night": "Evandro (19h às 07h)"}
                }
            },
            {
                "code": "HPV01",
            ...
            },
            {
                ...
                "schedule": {
                    "even": {"day": "Raynan (07h às 19h)", "night": "Kevin (19h às 07h)"},
                    "odd": {"day": "Guigelson (07h às 19h)", "night": "Agnaldo (19h às 07h)"}
                }
            },
            {
                "code": "HSM07",
            ...
            },
            {
                ...
                "schedule": {
                    "even": {"day": "Donizete (07h às 19h)", "night": "Sílvio (19h às 07h)"},
                    "odd": {"day": "Felipe (07h às 19h)", "night": "Alexandre (19h às 07h)"}
                }
            },
            {
                "code": "HTQ01",
    ...
    "Sul": {
        "PR": [
            ...
            },
            {
                ...
                "schedule": {
                    "even": {"day": "Marcos (07h às 19h)", "night": "Paulo (19h às 07h)"},
                    "odd": {"day": "Edvaldo (07h às 19h)", "night": "Augusto (19h às 07h)"}
                }
            },
            {
                "code": "HLO01",
            ...
            },
            {
                ...
                "schedule": {
                    "even": {"day": "Lucas (07h às 19h)", "night": "Josival (19h às 07h)"},
                    "odd": {"day": "Michelangelo (07h às 19h)", "night": "Valmir (19h às 07h)"}
                }
            },
            {
                "code": "HNE01",
                ...
                "schedule": {
                    "even": {"day": "Junior (07h às 19h)", "night": "Alex (19h às 07h)"},
                    "odd": {"day": "Iggor (07h às 19h)", "night": "André (19h às 07h)"}
                }
            }
        ],
        "RS": [
...
# ==============================================================================
def load_occurrences_data(filename):
    ...
    try:
        ...
        print(f"AVISO: Arquivo '{filename}' estava corrompido. Criando um novo arquivo de ocorrências.")
        os.rename(filename, f"{filename}.corrupted.{datetime.now().strftime('%Y%m%d%H%M%S')}")
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump([], f, indent=4, ensure_ascii=False)
        return []

def save_occurrences_data(data, filename):
    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=4, ensure_ascii=False)

occurrences = load_occurrences_data(OCCURRENCES_FILE)
...
@app.route('/')
def home():
    return jsonify({"message": "Servidor Flask MOVISAFE AMLAT está rodando com sucesso!"})

# Rota para obter as localizações
@app.route('/api/locations', methods=['GET'])
def get_locations():
    """Retorna todos os dados de localizações estáticos."""
    return jsonify(LOCATIONS_DATA)

# Rota para obter os dados de risco
@app.route('/api/risk_data', methods=['GET'])
def get_risk_data():
    """Retorna todos os dados de risco estáticos."""
    return jsonify(RISK_DATA_STATIC)
...
# Rota para adicionar uma nova ocorrência
@app.route('/api/occurrences', methods=['POST'])
def add_occurrence():
    """Adiciona uma nova ocorrência."""
    new_occurrence = request.json
    if not new_occurrence:
        return jsonify({"message": "Dados da ocorrência não fornecidos"}), 400

    new_occurrence['id'] = str(int(datetime.now().timestamp() * 1000))
    new_occurrence['timestamp'] = datetime.now().isoformat()

    occurrences.append(new_occurrence)
    save_occurrences_data(occurrences, OCCURRENCES_FILE)
    return jsonify(new_occurrence), 201

# Rota para excluir uma ocorrência
@app.route('/api/occurrences/<string:occurrence_id>', methods=['DELETE'])
def delete_occurrence(occurrence_id):
    """Exclui uma ocorrência pelo ID."""
    global occurrences
    initial_len = len(occurrences)
    occurrences = [o for o in occurrences if o.get('id') != occurrence_id]
    
    if len(occurrences) == initial_len:
        return jsonify({"message": "Ocorrência não encontrada"}), 404

    save_occurrences_data(occurrences, OCCURRENCES_FILE)
    return jsonify({"message": "Ocorrência excluída com sucesso"}), 200
